...
def insert_tags(df):
    ...
    # )
    df.tagged_text = '[CLS] ' + df.tagged_text + ' [SEP]'
    return df
...
# dr_trn_df = double_redact_tagged_finnum_df(
    # trn_df, {'xxtag', 'xxnum', '[CLS]', '[SEP]'})

vld_df = pd.read_json('/root/FinNum-3_dev.json')
vld_df = insert_tags(vld_df)
vld_df = vld_df.assign(is_vld=True)

#test set no category  
tst_df = pd.read_json('/root/FinNum-3_test.json')
...
dblock = DataBlock(
    blocks=(
        HF_TextBlock(hf_arch=hf_arch, hf_tokenizer=hf_tokenizer, hf_config =hf_config,hf_model =hf_model, max_length = 512, truncation = True, padding = True),
        CategoryBlock),
...
torch.save(dls, '/root/FinNum3_train_ZH_MacBERT.pth')

torch.save(
    dls.test_dl(tst_df.tagged_text.tolist(), bs=256),
    '/root/finnum3_test_dl.pth'
)
...
#     ),
#     '/root/finnum2_dls-dr-b8.pth'
# )

class_weights = torch.FloatTensor([0.31, 1]).cuda() # 999/3220 (out/in)
lrnr = Learner(
    torch.load('/root/FinNum3_train_ZH_MacBERT.pth'),
...
reset_randomness()
lr = 1.2e-05
lrnr.fit_one_cycle(3, lr_max = slice(1.7e-7, lr)) # 簡化 # 手動除在改數字

lr2 = 6e-06  # lr/2
lrnr.fit_one_cycle(3, lr_max = slice(8.6e-8, lr2))

# lr3 = 3e-06  # lr2/2
# lrnr.fit_one_cycle(1, lr_max = slice(4.3e-8, lr3))

# lr4 = 6e-07  # lr3/5
# lrnr.fit_one_cycle(1, lr_max = slice(8.6e-9, lr4))

# lrnr.unfreeze()

"""--------------------------------------------------------"""

# # lrnr.save('/root/fn_cf-b8-frozen-cl1_lr0.0014454397605732083')

# # reset_randomness()
# # _, _, preds = lrnr.get_preds(with_decoded=True)
# # vld_df.assign(prediction=preds.tolist())[['text', 'target_numeral', 'offset', 'category', 'claim', 'prediction']].to_json('/root/dev_20220108_2.json',orient='records')  #,indent=2
# reset_randomness()
# tst_dl = torch.load('/root/finnum3_test_dl.pth')
# _, _, preds = lrnr.get_preds(dl=tst_dl, with_decoded=True)
# tst_df.assign(prediction=preds.tolist())[['text', 'target_numeral', 'offset', 'prediction']].to_json('/root/tst_20220108_2.json',orient='records')  #,indent=2