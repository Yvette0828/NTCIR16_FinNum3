...
torch.save(
    ...
    '/root/NTCIR_FinNum3/ZH/Model/finnum3_test_dl.pth'
)
...
class_weights = torch.FloatTensor([0.31, 1]).cuda() # 999/3220 (in/out)
lrnr = Learner(
...
reset_randomness()
lr = 4.8e-04
lrnr.fit_one_cycle(2, lr_max = slice(6.8e-06, lr)) # 簡化 # 手動除再改數字 #5.6e-6

lr2 = 2.4e-04  # lr/2
lrnr.fit_one_cycle(3, lr_max = slice(3.4e-06, lr2)) #2.9e-6

lr3 = 1.2e-04  # lr2/2
lrnr.fit_one_cycle(1, lr_max = slice(1.7e-06, lr3))

lr4 = 2.4e-05  # lr3/5
lrnr.fit_one_cycle(1, lr_max = slice(3.4e-07, lr4))
...
vld_df.assign(prediction=preds.tolist())[['text', 'target_numeral', 'offset', 'category', 'claim', 'prediction']].to_json('/root/NTCIR_FinNum3/ZH/Result/FinNum3_zh_dev_baseline.json',orient='records')  #,indent=2

# reset_randomness()
tst_dl = torch.load('/root/finnum3_test_dl.pth')
_, _, preds = lrnr.get_preds(dl=tst_dl, with_decoded=True)
tst_df.assign(prediction=preds.tolist())[['text', 'target_numeral', 'offset', 'prediction']].to_json('/root/NTCIR_FinNum3/ZH/Result/FinNum3_zh_test_baseline.json',orient='records')  #, indent=2