...
# translated trainset
trans_trn_json_f = open('/root/NTCIR_FinNum3/ZH/Dataset/FinNum-3_translated_trn.json')
trans_trn_json = json.load(trans_trn_json_f)
# trans_trn_json.close()

trn_df = pd.read_json('/root/NTCIR_FinNum3/ZH/Dataset/FinNum-3_train.json')
trans_trn_df = pd.read_json('/root/NTCIR_FinNum3/ZH/Dataset/FinNum-3_translated_trn.json')

trn_df = trn_df.append(trans_trn_df, ignore_index=True)

"""# EDA for FinNum-3 Training Set
...
reset_randomness()
lr = 8.3e-04
lrnr.fit_one_cycle(2, lr_max = slice(1.2e-05, lr))

lr2 = 4.2e-04  # lr/2
lrnr.fit_one_cycle(3, lr_max = slice(6e-06, lr2))

lr3 = 2.1e-04  # lr2/2
lrnr.fit_one_cycle(1, lr_max = slice(3e-06, lr3))

lr4 = 4.2e-05  # lr3/5
lrnr.fit_one_cycle(1, lr_max = slice(6e-07, lr4))

# lr5 = 8.4e-06  # lr4/5
# lrnr.fit_one_cycle(1, lr_max = slice(1.2e-07, lr5))
...
vld_df.assign(prediction=preds.tolist())[[
    ...
    '/root/NTCIR_FinNum3/ZH/Result/FinNum3_zh_dev_aug.json',
    orient='records')
...
tst_df.assign(prediction=preds.tolist())[[
    ...
    '/root/NTCIR_FinNum3/ZH/Result/FinNum3_zh_test_aug.json',
